...
public class RemoveBookTest{
	...
	public void removeBookTestGUI() throws SQLException{
		bookController.addBook(b1, pdf, docx, fb2);
		bookController.displayResults(brief, title, language, keyword.get(0), authors.get(0), appendix, scope.get(0));
		assertEquals(panelTest.data.length,1);
		panelTest.row = 0;
		panelTest.btnRemoveBook.doClick();
		assertFalse(DBController.existsInDB("select * from book b where b.bookId = '"+bid+"';"));
	}
